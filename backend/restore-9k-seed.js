...
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var targetTenantId, locationId, jsonRaw, data, taxonomy_nodes, item_masters, item_variants, defaultCategoryId, _i, taxonomy_nodes_1, node, _a, taxonomy_nodes_2, node, e_1, masterMap, _b, item_masters_1, m, BATCH_SIZE, productsBatch, projectionsBatch, inserted, _c, item_variants_1, variant, master, attrs, variantSuffix, name_1, catId, itemType, sku, barcode;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    targetTenantId = "04bbc0e0-213d-4af4-9ce8-0e4674a58a90";
                    locationId = "loc-bali";
                    console.log("Starting massive 9K seed for tenant: ".concat(targetTenantId));
                    jsonRaw = fs.readFileSync("../seed-trial/universal_tenant_seed.json", "utf-8");
                    data = JSON.parse(jsonRaw);
                    taxonomy_nodes = data.taxonomy_nodes, item_masters = data.item_masters, item_variants = data.item_variants;
                    // 1. Map Taxonomy -> ProductCategory
                    console.log("Seeding ".concat(taxonomy_nodes.length, " Categories..."));
                    defaultCategoryId = "";
                ...
                case 10:
                    masterMap = new Map();
                    for (_b = 0, item_masters_1 = item_masters; _b < item_masters_1.length; _b++) {
                        m = item_masters_1[_b];
                        masterMap.set(m.id, m);
                    }
                    // 2. Map Variants -> Product & ProductProjection
                    console.log("Seeding ".concat(item_variants.length, " Products and Projections..."));
                    BATCH_SIZE = 500;
                    productsBatch = [];
                    projectionsBatch = [];
                    inserted = 0;
                ...
                case 11:
                    ...
                    catId = (master.taxonomy_node_ids && master.taxonomy_node_ids.length > 0)
                    ...
                    // @ts-ignore
                    return [4 /*yield*/, prisma.product.createMany({ data: productsBatch, skipDuplicates: true })];
                case 12:
                    // @ts-ignore
                    _d.sent();
                    // @ts-ignore
                    return [4 /*yield*/, prisma.productProjection.createMany({ data: projectionsBatch, skipDuplicates: true })];
                case 13:
                    // @ts-ignore
                    _d.sent();
                    inserted += productsBatch.length;
                    console.log("Inserted ".concat(inserted, "/").concat(item_variants.length, " items..."));
                    productsBatch = [];
                    projectionsBatch = [];
                    _d.label = 14;
                case 14:
