...
export class RetailInfrastructureService {
  constructor(
    private readonly infraRepo: IRetailInfrastructureRepository
  ) {}
  ...
  async setNodeStatus(tenantId: string, nodeId: string, status: 'ACTIVE' | 'STANDBY' | 'DOWN'): Promise<RetailGatewayNode> {
