...
import { RetailStore, RetailProduct, RetailOrder, RetailShift, RetailOrderItem } from '../entities/retail.entity';
import {
  CreateStoreDto,
  UpdateStoreDto,
  CreateOrderDto,
...
export class RetailDbRepository implements IRetailRepository {
  ...
  async getStore(tenantId: string, storeId: string): Promise<RetailStore | null> {
    const store = await this.prisma.store.findFirst({
      where: { id: storeId, companyId: tenantId, deletedAt: null },
    });
    return store ? this.mapStore(store) : null;
  }

  async createStore(tenantId: string, data: CreateStoreDto): Promise<RetailStore> {
  ...
  async updateStore(tenantId: string, storeId: string, data: UpdateStoreDto): Promise<RetailStore> {
    const store = await this.prisma.store.update({
      where: { id: storeId, companyId: tenantId },
      data: {
        name: data.name,
        type: data.type,
        phone: data.phone,
  ...
  async createInventoryPool(tenantId: string, data: CreateInventoryPoolDto): Promise<any> {
  ...
  async getInventoryPool(tenantId: string, poolId: string): Promise<any | null> {
    return this.prisma.inventoryPool.findFirst({
      where: { id: poolId, companyId: tenantId, deletedAt: null },
  ...
  async deleteInventoryPool(tenantId: string, poolId: string): Promise<void> {
    await this.prisma.inventoryPool.update({
      where: { id: poolId, companyId: tenantId },
  ...
  async listEcommerceStores(tenantId: string): Promise<any[]> {
  ...
  async getEcommerceStore(tenantId: string, storeId: string): Promise<any | null> {
  ...
  async createEcommerceStore(tenantId: string, data: CreateEcommerceStoreDto): Promise<any> {
  ...
  async updateEcommerceStore(tenantId: string, storeId: string, data: UpdateEcommerceStoreDto): Promise<any> {
  ...
  async linkEcommerceToBranch(tenantId: string, ecommerceId: string, branchId: string): Promise<void> {
    await this.prisma.ecommerceConnector.update({
      where: { id: ecommerceId, companyId: tenantId },
      data: { branches: { connect: { id: branchId } } },
    });
  }

  async unlinkEcommerceFromBranch(tenantId: string, ecommerceId: string, branchId: string): Promise<void> {
    await this.prisma.ecommerceConnector.update({
      where: { id: ecommerceId, companyId: tenantId },
  ...
  async listOrders(tenantId: string, storeId?: string): Promise<RetailOrder[]> {
    ...
    return orders.map((o: PrismaOrder & { items: (PrismaOrderItem & { product: Product | null })[] }) => this.mapOrder(o));
  }
  ...
  async updateOrderStatus(tenantId: string, orderId: string, status: string, metadata?: any): Promise<RetailOrder> {
  ...
  async reserveStock(tenantId: string, productId: string, quantity: number): Promise<{ success: boolean; reservationId?: string }> {
    const product = await this.prisma.product.findFirst({ where: { id: productId, companyId: tenantId } });
    if (!product) return { success: false };
    return { success: true, reservationId: `res_${Date.now()}` };
  }

  async releaseStock(tenantId: string, productId: string, quantity: number): Promise<void> {
    return;
  }

  async checkStock(tenantId: string, productId: string): Promise<{ available: number; status: string }> {
    const product = await this.prisma.product.findFirst({ where: { id: productId, companyId: tenantId } });
    if (!product) return { available: 0, status: 'OUT_OF_STOCK' };
    const available = 100;
    return { available, status: available > 10 ? 'IN_STOCK' : available > 0 ? 'LOW_STOCK' : 'OUT_OF_STOCK' };
  }
  ...
  async getActiveShift(tenantId: string, storeId: string, employeeId: string): Promise<RetailShift | null> {
    const shift = await this.prisma.retailShift.findFirst({
      where: { companyId: tenantId, storeId, employeeId, status: 'open' },
    });
  ...
  async closeShift(tenantId: string, shiftId: string, data: CloseShiftDto): Promise<RetailShift> {
    const shift = await this.prisma.retailShift.update({
      ...
      data: { endTime: new Date(), closingCash: data.closingCash, status: 'closed', notes: data.notes },
    });
    return this.mapShift(shift);
  }

  async listShifts(tenantId: string): Promise<RetailShift[]> {
  ...
  async updatePromotion(tenantId: string, promotionId: string, data: any): Promise<any> {
    const promotion = await this.prisma.retailPromotion.update({
      where: { id: promotionId, companyId: tenantId },
      data: {
        status: data.status,
  ...
  async listChannels(tenantId: string): Promise<any[]> {
    ...
    return channels.map((c: RetailChannel) => {
      const credentials = c.credentials as { clientId?: string; clientSecret?: string; branchId?: string; gatewayUrl?: string; connector?: string } | null;
      return {
        id: c.id,
        tenant_id: c.companyId,
  ...
  async createChannel(tenantId: string, data: any): Promise<any> {
    ...
    const creds = channel.credentials as { clientId?: string; clientSecret?: string; branchId?: string; gatewayUrl?: string; connector?: string } | null;
    return {
      id: channel.id,
      tenant_id: channel.companyId,
  ...
  async updateChannel(tenantId: string, channelId: string, data: any): Promise<any> {
    const updates: any = {};
    ...
    const channel =
      Object.keys(updates).length === 0
        ? await this.prisma.retailChannel.findFirst({ where: { id: channelId, companyId: tenantId } })
        : await this.prisma.retailChannel.update({ where: { id: channelId, companyId: tenantId }, data: updates });

    if (!channel) throw new Error('Channel not found');

    const credentials = channel.credentials as { clientId?: string; clientSecret?: string; branchId?: string; gatewayUrl?: string; connector?: string } | null;
    return {
      id: channel.id,
      tenant_id: channel.companyId,
      branchId: credentials?.branchId,
  ...
  async deleteChannel(tenantId: string, channelId: string): Promise<{ success: boolean }> {
    await this.prisma.retailChannel.update({ where: { id: channelId, companyId: tenantId }, data: { status: 'inactive' } });
    return { success: true };
  }

  async syncChannel(tenantId: string, channelId: string): Promise<{ success: boolean }> {
    await this.prisma.retailChannel.update({ where: { id: channelId, companyId: tenantId }, data: { lastSyncAt: new Date() } });
    return { success: true };
  }

  async getChannelById(tenantId: string, channelId: string): Promise<any | null> {
    return this.prisma.retailChannel.findFirst({ where: { id: channelId, companyId: tenantId } });
  }

  async updateChannelCredentials(tenantId: string, channelId: string, credentials: any): Promise<any> {
    return this.prisma.retailChannel.update({ where: { id: channelId, companyId: tenantId }, data: { credentials } });
  }

  async findChannelByClientId(tenantId: string, clientId: string): Promise<any | null> {
    const channels = await this.prisma.retailChannel.findMany({ where: { companyId: tenantId } });
    return channels.find((c: RetailChannel) => {
  ...
  async listDevices(tenantId: string, storeId?: string): Promise<any[]> {
    const where: any = { companyId: tenantId };
    if (storeId) where.storeId = storeId;
    const devices = await this.prisma.pOSDevice.findMany({ where, orderBy: { createdAt: 'desc' } });
    return devices.map((d: any) => ({
      id: d.id,
      tenant_id: d.companyId,
      store_id: d.storeId,
      name: d.name,
      type: d.type,
  ...
  async pingDevice(tenantId: string, deviceId: string): Promise<{ success: boolean }> {
    await this.prisma.pOSDevice.update({ where: { id: deviceId, companyId: tenantId }, data: { updatedAt: new Date() } });
    return { success: true };
  }
  ...
  async processPayment(tenantId: string, orderId: string, data: { amount: number; method: string; shiftId?: string }): Promise<any> {
    const order = await this.prisma.retailOrder.update({
      where: { id: orderId, companyId: tenantId },
      data: { paymentMethod: data.method, status: 'paid' },
    });
    return { success: true, order_id: order.id, amount: Number(order.totalAmount), method: data.method };
  }

  async processReturn(tenantId: string, orderId: string, data: { itemIds: string[]; shiftId?: string }): Promise<{ success: boolean }> {
    await this.prisma.retailOrder.update({ where: { id: orderId, companyId: tenantId }, data: { status: 'refunded' } });
    return { success: true };
  }
  ...
  async submitOpname(tenantId: string, data: { storeId: string; adjustments: any[]; shiftId?: string }): Promise<{ success: boolean }> {
    return { success: true };
  }

  async receiveGoods(tenantId: string, data: { storeId: string; shipmentId: string; items: any[]; shiftId?: string }): Promise<{ success: boolean }> {
    return { success: true };
  }
  ...
  async findCustomerByEmail(tenantId: string, email: string): Promise<any | null> {
    return this.prisma.retailCustomer.findFirst({ where: { companyId: tenantId, email }, include: { auth: true } });
  }

  async findCustomerById(tenantId: string, customerId: string): Promise<any | null> {
    return this.prisma.retailCustomer.findFirst({ where: { companyId: tenantId, id: customerId }, include: { auth: true } });
  }
  ...
  async createCustomer(tenantId: string, data: any): Promise<any> {
    return this.prisma.retailCustomer.create({
      data: {
        ...
        email: data.email,
        phone: data.phone,
        auth: data.passwordHash ? { create: { passwordHash: data.passwordHash, passwordUpdatedAt: new Date() } } : undefined,
      },
    });
  }

  async updateCustomer(tenantId: string, customerId: string, data: any): Promise<any> {
    return this.prisma.retailCustomer.update({
      where: { id: customerId, companyId: tenantId },
      data: { name: data.name, email: data.email, phone: data.phone, points: data.points, tier: data.tier },
    });
  }
  ...
  async findCustomerSession(tenantId: string, tokenHash: string): Promise<any | null> {
    return this.prisma.retailCustomerSession.findFirst({
      where: { companyId: tenantId, tokenHash, revokedAt: null, expiresAt: { gt: new Date() } },
    });
  }

  async revokeCustomerSession(tenantId: string, tokenHash: string): Promise<void> {
    await this.prisma.retailCustomerSession.updateMany({
      where: { companyId: tenantId, tokenHash, revokedAt: null },
  ...
  async createCart(tenantId: string, customerId: string): Promise<any> {
    return this.prisma.retailCart.create({ data: { companyId: tenantId, customerId, status: 'active' } });
  }

  async updateCartItem(tenantId: string, cartId: string, productId: string, data: { quantity: number; unitPrice: number }): Promise<any> {
    return this.prisma.retailCartItem.upsert({
      where: { cartId_productId: { cartId, productId } },
      update: { quantity: data.quantity, unitPrice: data.unitPrice },
      create: { cartId, productId, quantity: data.quantity, unitPrice: data.unitPrice },
    });
  }

  async removeCartItem(tenantId: string, cartId: string, itemId: string): Promise<void> {
    await this.prisma.retailCartItem.deleteMany({ where: { id: itemId, cartId } });
  }
  ...
  async upsertWishlist(tenantId: string, customerId: string): Promise<any> {
    return this.prisma.retailWishlist.upsert({
      where: { customerId },
      update: {},
      create: { companyId: tenantId, customerId },
    });
  }

  async addWishlistItem(tenantId: string, wishlistId: string, productId: string): Promise<any> {
    const existing = await this.prisma.retailWishlistItem.findFirst({ where: { wishlistId, productId } });
    if (existing) return existing;
    return this.prisma.retailWishlistItem.create({ data: { wishlistId, productId } });
  }

  async removeWishlistItem(tenantId: string, wishlistId: string, itemId: string): Promise<void> {
    await this.prisma.retailWishlistItem.deleteMany({ where: { id: itemId, wishlistId } });
  }
  ...
  async logEvent(tenantId: string, data: any): Promise<any> {
    return this.prisma.auditLog.create({
      data: {
        ...
        action: data.type,
        entityType: 'event',
        entityId: data.audit?.traceId ?? createHash('md5').update(Date.now().toString()).digest('hex'),
        userId: data.actor?.id ?? 'anonymous',
        changes: data.payload as any,
        metadata: { scope: data.scope, timestamp: data.timestamp, actorType: data.actor?.type } as any,
        createdAt: new Date(),
      },
    });
  }
