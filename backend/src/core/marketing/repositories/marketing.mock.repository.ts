...
export class MarketingMockRepository extends IMarketingRepository {
  ...
  private findCampaign(tenantId: string, campaignId: string) {
    const campaign = this.getStore(tenantId).campaigns.find(
  ...
  private findExecution(tenantId: string, executionId: string) {
    const execution = this.getStore(tenantId).executions.find(
  ...
  private findLead(tenantId: string, leadId: string) {
    const lead = this.getStore(tenantId).leads.find(
      (item) => item.id === leadId,
    );
  ...
  private findWorkflow(tenantId: string, workflowId: string) {
    const workflow = this.getStore(tenantId).workflows.find(
  ...
  private findAccount(tenantId: string, accountId: string) {
    const account = this.getStore(tenantId).accounts.find(
  ...
  async getDashboard(tenantId: string): Promise<MarketingDashboard> {
    ...
    return {
      ...
      qualifiedLeads: store.leads.filter((item) => item.status === "qualified")
        .length,
      handoffReady: store.leads.filter(
        (item) => item.status === "handoff_ready",
      ).length,
      spendToDate,
      attributedRevenue,
  ...
  async captureLead(
    ...
    const store = this.getStore(tenantId);
    const dedupKey =
      `${dto.companyName}-${dto.email ?? dto.phone ?? dto.contactName}`
        .trim()
        .toLowerCase();
  ...
  async acknowledgeAlert(
    ...
    const alert = this.getStore(tenantId).alerts.find(
  ...
  async runHealthSweep(
    ...
    store.leads.forEach((lead) => {
      if (
        lead.status === "handoff_ready" &&
        lead.updatedAt.getTime() < now - 1000 * 60 * 60 * 4
      ) {
        this.createAlertIfMissing(tenantId, {
          tenantId,
          type: "handoff_delay",
