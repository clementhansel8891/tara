...
export class FinanceMockRepository extends IFinanceRepository {
  ...
  private initializeMockData() {
    // Basic mock initialization for core ledgers
     this.createMockLedgerEntries('tenant-001', 'location-001', [
      { amount: 50000, type: 'credit', description: 'Initial Capital', category: 'Equity' }
    ]);
  }

  private createMockLedgerEntries(tenantId: string, locationId: string, entries: any[]) {
  ...
  async getLedger(tenantId: string, locationId?: string): Promise<LedgerEntry[]> {
    return this.ledgerEntries.filter(e => e.tenantId === tenantId && (!locationId || e.locationId === locationId));
  }
  ...
  async getTransactionById(tenantId: string, transactionId: string): Promise<Transaction | null> {
      return this.transactions.find(t => t.tenantId === tenantId && t.id === transactionId) || null;
  }
  ...
  async createAsset(tenantId: string, asset: Partial<Asset>): Promise<Asset> {
    const newAsset = { ...asset, id: `ast-${Date.now()}`, status: 'DRAFT' } as Asset;
    this.assets.push(newAsset);
    return newAsset;
  }

  async updateAsset(tenantId: string, assetId: string, updates: Partial<Asset>): Promise<Asset | null> {
  ...
  async getCapexRequestById(tenantId: string, id: string): Promise<CapexRequest | null> {
  ...
  async createCapexRequest(tenantId: string, request: Partial<CapexRequest>): Promise<CapexRequest> {
      const newReq = { ...request, id: `cpx-${Date.now()}`, status: 'PENDING' } as CapexRequest;
      this.capexRequests.push(newReq);
      return newReq;
  }

  async updateCapexRequest(tenantId: string, id: string, updates: Partial<CapexRequest>): Promise<CapexRequest | null> {
  ...
  async setCapexBudget(tenantId: string, budget: FinanceCapexBudgetRow): Promise<void> {
      const idx = this.capexBudgets.findIndex(b => b.department === budget.department);
      if (idx !== -1) {
  ...
  async listAssetDepreciationEntries(tenantId: string, assetId?: string): Promise<AssetDepreciationEntry[]> {
      return this.depreciationEntries.filter(d => !assetId || d.assetId === assetId);
  }

  async createDepreciationEntry(tenantId: string, entry: Partial<AssetDepreciationEntry>): Promise<AssetDepreciationEntry> {
      const newEntry = { ...entry, id: `dep-${Date.now()}` } as AssetDepreciationEntry;
  ...
  async getAssetAuditPack(tenantId: string, assetId: string): Promise<AssetAuditPack> {
      return {
          assetId,
          capexRequest: this.capexRequests.find(c => c.assetDescription.includes(assetId)), // rough match
          depreciationEntries: this.depreciationEntries.filter(d => d.assetId === assetId),
  ...
  async createReceivable(tenantId: string, invoice: Partial<ReceivableInvoice>): Promise<ReceivableInvoice> {
      const newInv = { ...invoice, id: `inv-${Date.now()}`, status: 'DRAFT' } as ReceivableInvoice;
  ...
  async updateReceivable(tenantId: string, id: string, updates: Partial<ReceivableInvoice>): Promise<ReceivableInvoice | null> {
      ...
      if (idx === -1) return null;
      this.receivableInvoices[idx] = { ...this.receivableInvoices[idx], ...updates };
  ...
  async createPayable(tenantId: string, bill: Partial<PayableBill>): Promise<PayableBill> {
      const newBill = { ...bill, id: `bill-${Date.now()}`, status: 'RECEIVED' } as PayableBill;
  ...
  async createPaymentRequest(tenantId: string, request: Partial<PaymentRequest>): Promise<PaymentRequest> {
      const newPay = { ...request, id: `pay-${Date.now()}`, status: 'DRAFT' } as PaymentRequest;
  ...
  // Payroll
  async listPayrollEntries(tenantId: string, period?: string): Promise<PayrollEntry[]> {
      return this.payroll;
  }
