    ...
    <div className="px-5 py-6 space-y-6 animate-fade-in">
      ...
      <div className="surface-elevated p-5 space-y-3">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-4xl font-display font-semibold">{bal.remaining_days}</span>
          <span className="text-sm text-muted-foreground">/ {bal.total_entitlement} hari</span>
        </div>
        <div className="h-2 rounded-full bg-secondary overflow-hidden">
          <div className="h-full rounded-full bg-gold transition-all" style={{ width: `${(bal.remaining_days / bal.total_entitlement) * 100}%` }} />
        </div>
        <p className="text-2xs text-muted-foreground">{bal.used_days} hari terpakai tahun ini</p>
      </div>
          ...
          <div key={l.id} className="surface-elevated p-4">
            <div className="flex items-start justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-2xs text-muted-foreground">{l.total_days} hari • {l.start_date && new Date(l.start_date).toLocaleDateString("id-ID")}</p>
