...
import { AlertTriangle, Shield, Activity, TrendingDown, Server, Users, ShoppingCart, Wallet } from "lucide-react";
...
export default function RiskMatrix() {
  const riskScore = riskData.filter(r => r.status === "active").length;
  const mitigated = riskData.filter(r => r.status === "mitigated").length;
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-black text-foreground">{riskData.filter(r => r.status === "monitoring").length}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          <div className="space-y-4">
            {riskData.map((risk, i) => (
              <div key={i} className={cn("p-5 rounded-xl border transition-all hover:shadow-md", statusColor[risk.status], "bg-card")}>
