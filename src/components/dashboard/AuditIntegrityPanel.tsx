...
export const AuditIntegrityPanel: React.FC<AuditIntegrityPanelProps> = ({ data, onVerify, loading }) => {
  return (
    <div className="flex flex-col h-full rounded-[2.5rem] border border-slate-800 bg-slate-900 p-8 shadow-2xl transition-all duration-500 hover:shadow-indigo-500/10 overflow-hidden group">
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Cryptographic verification of system logs</p>
          </div>
        </div>
        <Button 
          variant="ghost" 
          ...
          disabled={loading}
          onClick={onVerify}
          className="h-10 rounded-xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-white/10 hover:text-white transition-all"
        >
      ...
      <div className="flex flex-col md:flex-row items-center gap-12 py-2">
        <div className="relative flex flex-col items-center">
          <div className={cn(
            ...
            data.status === 'CLEAN' ? 'border-emerald-500/10 text-emerald-500 shadow-emerald-500/30' : 
            data.status === 'WARNINGS' ? 'border-amber-500/10 text-amber-500 shadow-amber-500/20' : 
            'border-rose-500/10 text-rose-500 shadow-rose-500/20'
          )}>
          ...
          <div className="mt-6 text-center">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mt-1">Integrity Score</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-2 group/stat">
              <div className={cn(
                ...
                data.brokenCount > 0 ? "text-rose-500" : "text-slate-500 group-hover/stat:text-rose-400"
              )}>
                <Unlock className="h-3 w-3" />
                <span className="text-[10px] font-black uppercase tracking-widest">Broken</span>
              </div>
              <p className={cn("text-3xl font-black tracking-tighter", data.brokenCount > 0 ? "text-rose-500" : "text-white")}>
                {data.brokenCount}
              </p>
            </div>
          </div>
      ...
      {/* Subtle corner glow */}
      <div className="absolute -top-24 -left-24 h-48 w-48 rounded-full bg-indigo-500/5 blur-[80px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
    </div>
  );
};
