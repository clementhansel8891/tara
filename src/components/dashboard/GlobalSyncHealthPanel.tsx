    ...
    <div className="flex flex-col h-full rounded-[2.5rem] border border-slate-800 bg-slate-900 p-8 shadow-2xl transition-all duration-500 hover:shadow-indigo-500/10 overflow-hidden group">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Database replication and outbox event integrity</p>
        </div>
        <div className={cn(
          "h-12 w-12 rounded-2xl flex items-center justify-center border transition-all duration-700",
          data.isHealthy ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-rose-500/10 border-rose-500/20 text-rose-400"
        )}>
      ...
      <div className="flex flex-col md:flex-row items-center gap-10 py-2">
        <div className="relative flex flex-col items-center gap-4">
          <div className={cn(
            ...
            data.isHealthy ? "border-emerald-500/10 text-emerald-500 shadow-emerald-500/20" : "border-rose-500/10 text-rose-500 shadow-rose-500/20"
          )}>
          ...
          <div className="flex flex-col items-center">
            <span className={cn(
              ...
              data.isHealthy ? "text-emerald-400" : "text-rose-400"
            )}>
              {data.isHealthy ? 'Operational' : 'Sync Latency'}
            </span>
            <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Status</span>
          </div>
        </div>
        ...
        <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="space-y-3 rounded-3xl bg-white/5 p-6 border border-white/5 group-hover:border-white/10 transition-colors">
            <div className="flex items-center justify-between text-slate-500">
              ...
              <RefreshCcw className="h-4 w-4 animate-spin-slow text-indigo-400" />
            </div>
          ...
          <div className={cn(
            ...
            data.failed > 0 ? "bg-rose-500/10 border-rose-500/20" : "bg-white/5 border-white/5"
          )}>
            <div className="flex items-center justify-between text-slate-500">
              <span className="text-[10px] font-black uppercase tracking-widest">Failed</span>
              <AlertTriangle className={cn("h-4 w-4", data.failed > 0 ? "text-rose-500 animate-bounce" : "text-slate-600")} />
            </div>
            <p className={cn("text-4xl font-black tracking-tighter", data.failed > 0 ? "text-rose-500" : "text-white")}>{data.failed}</p>
          </div>
          ...
          <div className="col-span-1 sm:col-span-2 flex flex-col sm:flex-row items-center justify-between gap-4 px-2 pt-4 border-t border-white/5">
            <div className="flex items-center gap-3">
              <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgb(16,185,129)]" />
      ...
      <div className="absolute -bottom-20 -right-20 h-48 w-48 rounded-full bg-indigo-500/5 blur-[80px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
    </div>
  );
};
